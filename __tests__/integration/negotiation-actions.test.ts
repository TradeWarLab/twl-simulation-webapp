...
import { createAsk, createBundle, respondToBundle } from "@/app/actions/negotiations";
...
describe("Negotiation Actions", () => {
  ...
  describe("createBundle", () => {
    it("creates bundle and links actions", async () => {
      ...
      const bundleSelectMock = vi.fn().mockReturnValue({ single: bundleSingleMock });
      const bundleInsertMock = vi.fn().mockReturnValue({ select: bundleSelectMock });
      ...
      const result = await createBundle("class-1", "team-a", ["act-1", "act-2"]);
