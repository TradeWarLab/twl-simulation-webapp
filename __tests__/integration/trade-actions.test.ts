...
describe("Trade Actions", () => {
  ...
  describe("initializeTradeItems", () => {
    it("returns error if unauthorized", async () => {
      ...
      const result = await initializeTradeItems("c1", "t1", [{ name: "A", value: 1 }]);
      expect(result).toEqual({ error: "Unauthorized" });
    });
    ...
    it("inserts multiple items and revalidates path", async () => {
      ...
      const result = await initializeTradeItems("class-9", "team-x", itemsToInit);
      ...
      expect(result).toEqual({ success: true });
      expect(revalidatePath).toHaveBeenCalledWith("/instructor/classes/class-9");
    });
  });
});
