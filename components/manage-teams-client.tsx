...
export function ManageTeamsClient({ classId, initialRoster }: { classId: string, initialRoster: ClassRosterEntry[] }) {
    ...
    const handleAffiliationChange = async (entry: ClassRosterEntry, newAffiliation: "USA" | "China") => {
        if (entry.affiliation === newAffiliation) return;

        // Optimistically update
        const previousRoster = [...roster];
        setRoster(r => r.map(st => 
    ...
    const handleInterestChange = async (entry: ClassRosterEntry, newInterest: string) => {
        ...
        <div className="rounded-md border overflow-hidden">
            ...
            {roster.length === 0 ? (
                ...
                <div className="divide-y relative">
                    ...
                    <div className={`absolute inset-0 bg-white/50 z-10 transition-opacity ${isPending ? 'opacity-100 pointer-events-none' : 'opacity-0 pointer-events-none'}`} />
                    
                    {roster.map((entry) => (
                        <div key={entry.email} className={`grid grid-cols-12 p-4 text-sm items-center transition-colors ${entry.affiliation === 'USA' ? 'hover:bg-blue-50 dark:hover:bg-blue-900/10' : 'hover:bg-red-50 dark:hover:bg-red-900/10'}`}>
