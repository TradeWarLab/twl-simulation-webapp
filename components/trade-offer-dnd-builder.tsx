...
type ContainerId = "my-inventory" | "offer-zone" | "opponent-inventory" | "request-zone";
...
export function TradeOfferDndBuilder({
  ...
  const [liveOpponentItems, setLiveOpponentItems] = useState<TradeItem[]>(opponentTeamItems);
  const [myInventory, setMyInventory] = useState<TradeItem[]>(myTeamItems);
  const [opponentInventory, setOpponentInventory] = useState<TradeItem[]>(opponentTeamItems);
  ...
  const itemMeta = useMemo(() => {
    ...
    for (const i of liveOpponentItems) map.set(i.id, { item: i, side: "opponent" });
    return map;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "trade_items", filter: `class_id=eq.${classId}` },
        (payload) => {
          ...
          if (next.team_id === myTeamId) {
            ...
            setLiveOpponentItems((prev) => prev.filter((i) => i.id !== next.id));
  ...
  useEffect(() => {
    ...
    setOfferItems((prev) => prev.map((i) => myById.get(i.id)).filter(Boolean) as TradeItem[]);
    setRequestItems((prev) => prev.map((i) => oppById.get(i.id)).filter(Boolean) as TradeItem[]);
  }, [liveMyItems, liveOpponentItems]);
  ...
  useEffect(() => {
    setMyInventory((prev) => {
      ...
      next.sort((a, b) => (orderIndex.get(a.id) ?? 1e9) - (orderIndex.get(b.id) ?? 1e9));
      return next;
    });
  ...
  function getContainerForItemId(itemId: string): ContainerId | null {
    ...
    if (opponentInventory.some((i) => i.id === itemId)) return "opponent-inventory";
  ...
  function moveItem(itemId: string, from: ContainerId, to: ContainerId) {
    ...
    const removeFrom = (container: ContainerId) => {
      if (container === "my-inventory") setMyInventory((p) => arrayRemove(p, (x) => x.id === itemId));
      if (container === "offer-zone") setOfferItems((p) => arrayRemove(p, (x) => x.id === itemId));
      if (container === "opponent-inventory")
        setOpponentInventory((p) => arrayRemove(p, (x) => x.id === itemId));
      if (container === "request-zone") setRequestItems((p) => arrayRemove(p, (x) => x.id === itemId));
    };
    const addTo = (container: ContainerId) => {
      if (container === "my-inventory") setMyInventory((p) => arrayUpsertMove(p, item, (x) => x.id));
      if (container === "offer-zone") setOfferItems((p) => arrayUpsertMove(p, item, (x) => x.id));
      if (container === "opponent-inventory")
        setOpponentInventory((p) => arrayUpsertMove(p, item, (x) => x.id));
      if (container === "request-zone") setRequestItems((p) => arrayUpsertMove(p, item, (x) => x.id));
    };
  ...
  const activeItem = activeId ? itemMeta.get(String(activeId))?.item ?? null : null;
  const activeSide = activeItem ? (liveMyItems.some((i) => i.id === activeItem.id) ? "my" : "opponent") : null;
  ...
  return (
    <div className="space-y-3">
        ...
        <span className="inline-flex items-center gap-1.5">
          <span
            className={[
              ...
              liveStatus === "live" ? "bg-emerald-500" : liveStatus === "connecting" ? "bg-amber-500" : "bg-red-500",
            ].join(" ")}
          />
          <span className="text-muted-foreground">
            {isSyncing ? "Syncing..." : liveStatus === "live" ? "Live" : liveStatus === "connecting" ? "Connecting..." : "Offline"}
          </span>
        </span>
      </div>
                ...
                <InventoryContainer id="my-inventory" tone="my">
                  <div className="flex flex-wrap gap-2 p-2">
                    {myInventory.length === 0 ? (
                      <p className="text-xs text-muted-foreground italic p-2">No items available</p>
                    ) : (
            ...
            >
              {offerItems.map((item) => (
                <TradeItemCard
                  ...
                  onRemove={() => moveItem(item.id, "offer-zone", "my-inventory")}
                />
              ))}
            </DropZone>
          </div>
            ...
            >
              {requestItems.map((item) => (
                <TradeItemCard
                  ...
                  onRemove={() => moveItem(item.id, "request-zone", "opponent-inventory")}
                />
              ))}
            </DropZone>
          </div>
          ...
          {activeItem && activeSide ? (
            <div className="pointer-events-none">
              <TradeItemCard
                ...
                side={activeSide}
                variant="inventory"
                containerId={activeSide === "my" ? "my-inventory" : "opponent-inventory"}
              />
            </div>
          ) : null}
        </DragOverlay>
      ...
      <Button
        type="button"
        onClick={submit}
        disabled={isPending || (offerItems.length === 0 && requestItems.length === 0)}
