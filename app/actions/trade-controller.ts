 ...
 */
export async function getTradeProposals(classId: string): Promise<TradeProposal[]> {
...
export async function createTradeProposal(
    ...
    if (!classData || classData.current_period !== 3) {
        return { error: "Trade proposals can only be created during the Negotiation phase" };
    }
...
export async function executeTrade(proposalId: string) {
    ...
    for (const item of requestedItems) {
        ...
        if (error) {
            console.error(`Error transferring requested item ${item.item_id}:`, error);
            return { error: `Failed to transfer item: ${item.name}` };
        }
    }
