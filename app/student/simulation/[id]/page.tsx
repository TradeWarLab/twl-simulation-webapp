...
async function SimulationPageInner({ params }: { params: Promise<{ id: string }> }) {
    ...
    const briefings = teamRecord 
        ? await getStudentBriefings(id, teamRecord.country, enrollment.interest_block)
        : [];
            ...
            {!teamRecord ? (
                <main className="flex-1 flex items-center justify-center">
                    <Card className="w-full max-w-md p-6 text-center">
                        <CardHeader>
                            <CardTitle className="text-2xl text-slate-700">Waiting for Assignment</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <p className="text-muted-foreground">Your instructor has not yet assigned you to a team. Please wait.</p>
                        </CardContent>
                    </Card>
                </main>
            ) : (
                            ...
                            {briefings.length === 0 ? (
                                ...
                                briefings.map(b => (
                                    <div key={b.id} className="p-3 bg-muted rounded-md text-sm">
                                        ...
                                        {b.content && <p className="whitespace-pre-wrap text-muted-foreground mb-2">{b.content}</p>}
                                        {b.file_url && (
